    ...
    <v-col>
      <v-sheet height="64">
        <v-toolbar flat color="white">
          <v-btn color="grey darken-2" class="mr-4" @click="dialog = true" dark>New Event</v-btn>
        ...
        <v-card>
          <v-container>
            <v-form @submit.prevent="addEvent">
              ...
              <v-text-field v-model="colour" type="color" label="Colour"></v-text-field>
              <v-btn
                type="submit"
                color="primary"
                class="mr-4"
      ...
      <v-sheet height="600">
        ...
        >
          <v-card color="grey lighten-4" min-width="350px" flat>
            <v-toolbar :color="selectedEvent.colour" dark>
...
export default {
  ...
  methods: {
    async getEvents() {
      let snapshot = await db.collection("event").get();
      let events = [];
    ...
    async updateEvent(ev) {
      await db.collection("event").doc(this.currentlyEditing).update({
        details: ev.details,
      });
